          ...
          <div className="flex items-center justify-between px-6 h-16">
            ...
            <nav className="hidden md:flex items-center">
              <ul className="flex space-x-4">
                {navigationItems.map((item) => (
                  <li key={item.name}>
                    <Link isBlock color="foreground" href={item.href} 
                        className={clsx(
            ...
            >
              <div className="w-6 flex flex-col gap-1.5">
                <span className={`block h-0.5 w-full bg-white transition-transform duration-300 ${mobileMenuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
                <span className={`block h-0.5 w-full bg-white transition-opacity duration-300 ${mobileMenuOpen ? 'opacity-0' : 'opacity-100'}`}></span>
                <span className={`block h-0.5 w-full bg-white transition-transform duration-300 ${mobileMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
              </div>
            </button>
