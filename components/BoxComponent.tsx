...
export default function BoxComponent({
  ...
  const [probabilidades, setProbabilidades] = useState<TierProbabilidad[]>(initialProbabilidades);
    ...
    try {
      ...
      if (matchingSkins.length > 0) {
        ...
        for (const skin of matchingSkins) {
          ...
          const tierData = supabase ? await getTierData(supabase, skin.contentTierUuid) : null;
  ...
  const loadProbabilidades = async () => {
    ...
    try {
      ...
      if (error) {
        if (error.message) {
          console.error('Error al obtener probabilidades:', error.message, error);
        } else {
          console.error('Error al obtener probabilidades:', JSON.stringify(error));
        }
        return;
      }
  ...
  const checkDailyOpened = async () => {
    if (!caja?.es_diaria || !isAuthenticated || !session?.user?.id || !supabase) return;
    ...
    try {
      ...
      console.error('Error al verificar si la caja diaria ya fue abierta:', error);
    }
  };
  ...
  const generateSpinItems = (selectedSkin: Skin) => {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      if (randomSkin) baseItems.push({...randomSkin, id: `${randomSkin.id}-${i}`});
    }
    ...
    for (let i = 0; i < 25; i++) {
      const randomSkin = selectRandomSkinByProbability(cajaSkins, probabilidades);
      if (randomSkin) baseItems.push({...randomSkin, id: `${randomSkin.id}-${i+25}`});
    }
  ...
  const animateSpinner = (finalPosition: number) => {
    ...
    const containerWidth = typeof window !== 'undefined' ? window.innerWidth : 800; // This is used for offset calculation
  ...
  const openBox = async () => {
    if (!caja || !caja.esta_disponible || isOpening || isSpinning || (caja.es_diaria && dailyOpened)) {
      if (caja.es_diaria && dailyOpened) console.log('La caja diaria ya fue abierta hoy');
      return;
    }
  ...
  useEffect(() => {
    if (isSpinning && spinItems.length > 0 && resultSkinForSpin && spinnerRef.current && spinnerRef.current.parentElement) {
    ...
    <div className="w-full">
      {resultSkin ? (
        ...
        <div className="p-8 rounded-xl max-w-md w-full text-center">
          <h3 className="text-3xl font-bold text-white mb-2">¡Recompensa obtenida!</h3>
          ...
          <p className="text-xl font-semibold text-white mb-2">{resultSkin.nombre}</p>
        ...
        <div className="flex flex-col items-center w-full">
          {caja && (
            <div className="text-center">
              ...
              {isSpinning ? (
                ...
                <div className="w-full text-center mb-8 relative"> {/* Added relative positioning for the marker */}
                  <div 
                    ...
                    >
                      {spinItems.map((skin, index) => (
                        ...
                        >
                          ...
                          <p className="text-sm text-white truncate mt-1 w-full">{skin.nombre}</p>
                        </div>
                      ))}
                    </div>
                  ...
                  <p className="text-lg text-white/70 mt-4">La caja está girando...</p>
                </div>
              ) : (
              ...
              {caja.descripcion && <p className="text-white/80 max-w-md mb-6 mx-auto">{caja.descripcion}</p>}
              {caja.es_diaria && nextUpdate && (
                <div className="mb-6 text-white/70">
                  <p>Próxima actualización: <span className="text-primary">{nextUpdate}</span></p>
                </div>
              )}
              ...
              <div className="flex gap-4 mb-6 justify-center">
                ...
                >
                  ...
                  {isOpening || isSpinning
                    ...
                    : caja.es_diaria
                      ? dailyOpened ? 'Ya abierta hoy' : 'Abrir Caja'
              ...
              {showProbabilities && probabilidades && (
                <div className="w-full bg-black/20 rounded-lg p-4 border border-white/10 max-w-md mx-auto mt-4">
                  <h4 className="text-lg font-medium text-white mb-3">Probabilidades de obtención</h4>
                        ...
                        <div key={prob.id} className="text-left">
                          <div className="flex justify-between items-center">
                            <span className="text-white/90 flex items-center">
                              {prob.content_tier?.color && <div className="w-3 h-3 rounded-full mr-2" style={{ backgroundColor: prob.content_tier.color }}></div>}
          ...
          {!caja && <p>Cargando información de la caja...</p>} {/* Should ideally not be reached due to early returns */}
        </div>
      )}
    </div>
  );
